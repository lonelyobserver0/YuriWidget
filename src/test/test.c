...
static void on_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    gtk_widget_hide(label); // Nascondi l'etichetta
}
...
static gboolean on_key_press(GtkWidget *widget, GdkEventKey *event, gpointer data) {
    ...
    if (event->keyval == GDK_KEY_Escape) {
        gtk_widget_hide(label); // Nascondi l'etichetta se premi "Esc"
...
//    const char *url = argv[6];
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(on_button_clicked), main_window);
    g_signal_connect(window, "key-press-event", G_CALLBACK(on_key_press), main_window);
